...
define('MYSQL_ASSOC',MYSQLI_ASSOC);
  ...
  class Database
  {
       ...
       */
      public function ignore($table, $data, $uniqueKey, $uniqueValue)
      {
		  global $core;
          ...
          if ($this->query($q)) {
            ...
            if ($this->insertid() == 0) {
                $id = $this->first("SELECT id FROM ".$table." WHERE ".$uniqueKey." = '".$uniqueValue."'");
                return $id["id"];
            } else {
       ...
       */
      public function duplicate($table, $data, $uniqueKey, $uniqueValue)
      {
		  global $core;
		  if ($table === null or empty($data) or !is_array($data)) {
		  $this->error("Invalid array for table: <b>".$table."</b>.");
          ...
          $q .= "(" . rtrim($k, ', ') . ") VALUES (" . rtrim($v, ', ') . ") ON DUPLICATE KEY UPDATE ".$f.";";
